...
% >> indivstats('individualstats_config.m')
%
% _________________________________________________________________________


function indivstats_spm12(Config_File)

global currentdir idata_type session_img;

Config_File = Config_File(1:end-2); %Lei Hao 2017/03/10
eval (Config_File); %Lei Hao 2017/03/10
postfix = paralist.postfix; %Lei Hao 2017/03/10
%exp_sesslist=strtrim(paralist.exp_sesslist); %Lei Hao 2017/03/10
...
fname = sprintf(['indivstats_',postfix,'-%d_%02d_%02d-%02d_%02d_%02.0f.log'],c); %Lei Hao 2017/03/10
diary(fname);
...
parent_folder = readlist_qin(parent_folder);
...
for subcnt = 1:numsub
    ...
    sub_stats_dir = fullfile(sub_dir, 'stats_spm12', stats_folder);
    ...
    for sesscnt = 1:numsess
        ...
        addpath(fullfile(session_raw_dir{sesscnt}, 'taskdesign'));%change to adopt muen's data
        str = which(task_dsgn);
        if isempty(str)
        ...
        if(include_mvmnt == 1)
            load task_design
            reg_file = spm_select('FPList', session_img_dir, '^rp_arI');
            unix(sprintf('gunzip -fq %s', reg_file));
            reg_file = spm_select('FPList', session_img_dir, '^rp_arI');
            if isempty(reg_file)
                reg_path = fullfile(session_raw_dir{sesscnt}, 'unnormalized');
                reg_file = spm_select('FPList', reg_path, '^rp_arI');
                unix(sprintf('gunzip -fq %s', reg_file));
                reg_file = spm_select('FPList', reg_path, '^rp_arI');
                if isempty(reg_file)
    ...
    if include_artrepair == 1
        addpath(genpath('/Users/hao1ei/Toolbox/spm12/toolbox/ArtRepair'));
...
load (fullfile (template_path,'firstlv_batch.mat')); % Lei Hao added
...
for sess = 1:numsess
    ...
    for nthfile = 1:nscans
        matlabbatch{1}.spm.stats.fmri_spec.sess(sess).scans{nthfile,1} = ...
            deblank(files(nthfile,:)); %haiyang edit, from 'scans{nthfile}' to 'scans{nthfile,1}' for solving cat different dimension error
    end
